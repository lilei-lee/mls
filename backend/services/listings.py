...
def create_listing(req, physical_attrs: dict, agent: dict) -> dict:
    ...
    Args:
        req: CreateListingRequest (营销字段)
        physical_attrs: {area_sqm, floor, total_floor, rooms, bathrooms}（从 PostListingRequest 提取）
        agent: 当前登录经纪人 dict

    Returns:
        {'listing_id': str, 'house_code': str}

    Raises:
        409 — house_code 重复（该房已被其他人录入）
        400 — 超过 MAX_PHOTOS(6) 张照片
        503 — 辞典微服务不可达
        409 — 辞典 claim 冲突（physical_attrs 与已有数据不一致）
...
def list_my_listings(agent_id: ObjectId, skip: int = 0, limit: int = 20) -> list:
    """LA 视角：我的挂牌房源列表。按 updated_at 降序，分页。"""
    cursor = (
...
def get_listing_by_id(listing_id: str, viewer_id=None) -> dict | None:
    """获取单套房源详情（decorated full view）。

    权限控制：
    - 房主 (owner_agent_id == viewer_id): 返回完整字段
    - 非房主: building/unit/room_no/agent_remarks/showing_instructions 脱敏为空
    - 返回 None 表示不存在或已下架（非房主看不到 offline）

    封面回退规则：cover_thumbnail 如果为空且 photos[0] 有 photo_key，
    自动推导为 /api/v1/photos/{key}（_derive_cover_thumbnail）。
    """
    try:
