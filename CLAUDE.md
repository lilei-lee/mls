...
| 模块六 Web 管理后台 | FastAPI+Jinja2 服务端渲染(`routers/admin.py`+`admin_auth.py`+`templates/admin/`，`/admin/login`，自签名 cookie)：**管理员登录 + 数据看板(含环比/区域/门店) + 经纪人管理(列表/详情/联卖审核) + 房源管理(列表/筛选/详情/手动下架恢复) + 会员管理(grant/续期)** | 小区库维护、争议仲裁、系统配置、数据导出、**审计日志查看(前置:先建 audit_log 写入机制,当前无)**、经纪人暂停/踢出(前置:enforcement 语义+audit_log)（module_6_admin.md 全设计） |
| 模块七 推送消息 | **未启动** | 全部 |
