...
> **代确认成交(模块六,唯一进交易的后台写口)**:仅当 LA 被暂停/踢出导致 pending_la_confirm 卡住时,admin 替 LA **独立填真实成交值**(表单盲填,不回显 BA 值),走**同一个 `_compare_and_finalize`**——分毫不差才 confirmed,否则 rejected。admin 不是"批准 BA 的数",比对机制一点不削弱。见 `transactions.proxy_confirm_transaction`。
...
| 模块六 Web 管理后台 | FastAPI+Jinja2 服务端渲染(`routers/admin.py`+`admin_auth.py`+`audit.py`+`templates/admin/`，`/admin/login`，自签名 cookie)：**多管理员+角色权限RBAC(`admin_users.py`:admin_users集合+bcrypt密码+种子超管;4角色 超级管理员/运营/风控审核/客服 → 功能区权限矩阵;require_admin 按请求路径做功能区级鉴权,无权跳/admin/denied;导航按权限过滤;登录日志admin_login_log+陌生IP标记;ActorContextMiddleware 把操作归属到具体管理员→审计 actor 记到人;超管可建号/改角色/停用启用/重置密码,护栏:不能停用最后一个超管或自己) + 管理员登录 + 数据看板(含环比/区域/门店) + 经纪人管理(列表/详情/联卖审核/会员开通续期/踢出恢复/暂停解除) + 房源管理(列表/筛选/详情/手动下架恢复) + 小区库管理(列表/编辑含11丰富参数+备案名+别名/合并/CSV批量导入导出;一个项目多名:标准名+备案名+别名数组,搜索三者都命中,merge 旧名自动入 aliases) + 审计日志(audit_log 写入+查看) + 数据导出(CSV) + 系统配置(`system_config.py`,真生效运营参数:带客过期天数/照片上限/密码锁定,已接入 scheduler/listings/auth) + 争议仲裁(`disputes.py`,经纪人举报+后台受理/裁决/驳回,处罚联动踢出) + 定金异常审查(deposit_events 监控,超 2 次标红) + 定金凭证审核(后台凭证巡检+合规判定+admin 图片代理,deposit_proof_url 后端已收) + 待确认成交管理(只读:待确认/价格不一致/已撤回,超6天标红) + 归属变更(`ownership.py`,发起锁定→复核→确认转移/驳回,全护栏:非sold/无进行中成交/目标active/二次校验;委托凭证强制可配 ownership_require_proof 种子期默认关) + 代确认成交(B模型,前置:仅LA被暂停/踢出卡住;盲填不回显BA值+走原比对,分毫不差才confirmed,不一致照样rejected——基石未穿透;重留痕)** | 代确认成交(⚠️碰反作弊基石,需专门设计)、争议补充材料多轮、系统配置 §8 其余项(对应功能未实现)（module_6_admin.md 全设计）<br>**经纪人暂停语义(已实现)**:suspended=软限制,可登录/读/完成在途协作(审批/确认带看/发起成交/填价/标记),但拦"发起新业务"(挂房源/带客申请/带看)——`auth.get_unsuspended_agent` 接入 4 个拉新端点;banned 优先于 suspended |
| 模块七 推送消息 | **未启动** | 全部 |
