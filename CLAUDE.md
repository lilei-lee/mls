...
| 模块六 Web 管理后台 | **第一片已落地**：FastAPI+Jinja2 服务端渲染 + 管理员登录(自签名 cookie,config.ADMIN_USERNAME/PASSWORD) + 数据看板 + 会员管理(Web 版 grant/续期)。`routers/admin.py` + `admin_auth.py` + `templates/admin/`，访问 `/admin/login` | 经纪人管理、房源审核、小区库维护、争议仲裁、系统配置、数据导出、审计日志查看（module_6_admin.md 全设计） |
| 模块七 推送消息 | **未启动** | 全部 |
